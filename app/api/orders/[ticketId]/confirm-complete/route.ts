...
export async function POST(_request: Request, { params }: { params: Promise<{ ticketId: string }> }) {
  ...
  try {
    ...
    await pool.request().input("OrderDetailId", sql.Int, orderDetailId).execute("usp_ConfirmOrderComplete");

    const updated = await fetchTicketAndOrder(pool, Number(ticketId));
    return NextResponse.json(updated);
  } catch (err) {
    console.error(`POST /api/orders/${ticketId}/confirm-complete failed:`, err);
    const message = err instanceof Error ? err.message : String(err);
    return NextResponse.json({ error: "Failed to confirm order complete", detail: message }, { status: 500 });
  }
}
